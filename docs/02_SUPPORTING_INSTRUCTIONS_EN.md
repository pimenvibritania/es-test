...
### 3.1 Secrets Manager + KMS: detailed behavior

- **Generation**: both passwords (`elastic` superuser, `kibana_system` service account) are generated by the `random` Terraform provider (`random_password`, length 20, special chars) — they never exist as literal strings anywhere in source code, only as Terraform resource state/output.
- **Storage**: each secret is its own `aws_secretsmanager_secret` + `aws_secretsmanager_secret_version` pair (`elastic-password`, `kibana-system-password`, `ca-bundle`). All three set `kms_key_id` to the same customer-managed CMK (`aws_kms_key.es_cmk`) — encryption-at-rest is explicit, not the default AWS-managed key.
- **KMS key reuse**: the same CMK also encrypts every node's root EBS volume (`root_block_device.kms_key_id`). One key, two purposes (EBS + Secrets Manager), simplifying key management for this exercise's scope.
- **Key rotation**: `enable_key_rotation = true` — AWS automatically rotates the underlying key material yearly; this is transparent (key ARN/alias unchanged, previously-encrypted data remains decryptable).
- **`recovery_window_in_days = 0`**: deliberately skips the default 30-day recovery window on the Secrets Manager secrets, so a `terraform destroy` → `apply` cycle can immediately recreate a secret with the same name (the default window blocks this with `InvalidRequestException`). Acceptable trade-off for a test/demo environment; a production secret handling real compliance/audit needs would keep the recovery window.
- **Access control is two-layered, not just "who has the URL"**: to actually read a secret's plaintext value, a caller needs (1) network path to the Secrets Manager API (any AWS SDK call, doesn't require VPC placement) **and** (2) an IAM identity explicitly granted `secretsmanager:GetSecretValue` on that specific secret ARN **and** `kms:Decrypt` on the CMK. Currently only the ES node role (`es_role`) and the Kibana instance role have this grant — no other principal in the account can read these secrets' values, even with `AdministratorAccess`-adjacent roles unless explicitly scoped in.

### 3.2 How a client application would consume ES (not implemented in this repo — architecture only)

This repository's scope is infrastructure provisioning, not a sample consumer application, so no application code ships here. The intended integration pattern for any future application connecting to this cluster:

1. **Network placement**: the app must run inside the same VPC (`10.1.0.0/16`) or connect through the Pritunl VPN — ES is never reachable from the public internet (security group only allows 9200 from `allowed_cidr`).
2. **Runtime credential fetch, never hardcoded**: the app's compute (EC2/Lambda/ECS task) would need its own IAM role granted `secretsmanager:GetSecretValue` on `elasticsearch/es-test/elastic-password` and `kms:Decrypt` on `aws_kms_key.es_cmk` — scoped to only that secret, following the same least-privilege pattern as `es_role`. This is a new IAM role/policy that would need to be added; it does not exist yet in this repo since no consumer app exists.
3. **At startup**, the app calls the AWS SDK (`secretsmanager.get_secret_value`) once to fetch the password into memory, then uses a standard ES client (e.g. Python `elasticsearch` package) with `basic_auth=("elastic", password)` against all 3 node HTTPS endpoints (the client library handles node failover automatically). The password never touches a config file, environment variable baked into an image, or a source-controlled file — it lives only in the running process's memory for its lifetime.
4. **No secret ever leaves AWS's secret store as plaintext at rest** — it's fetched over TLS from the Secrets Manager API directly into the app process, decrypted in-flight by KMS on AWS's side, and the app never writes it back to disk.
